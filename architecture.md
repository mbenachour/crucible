...
### 10.4 `crucible/api/` — the read-first HTTP API  · *foundations + read endpoints done (milestone: API, PR #47)*
...
| `routers/` | `health`, `runs` (+report/metrics/coverage), `findings` (+validations, cross-run `stable_key`), `artifacts` (+recon/dedup/log shortcuts), `state`, `wishlist` |

**Read-first.** The only write is `POST /wishes/{id}/resolve`, gated behind the
write token. Bearer-token auth (full + optional read-only) is off on a loopback
bind. **No outbound network calls** — data residency (§13) holds.
