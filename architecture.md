...
|---|---|---|
| `runs` | `Run` | `run_id` (pk), `repo_path`, `repo_commit`, `primary_language`, `status`, `workspace_path`, `finished_at`, `outcome` (`completed`/`stopped_at_stub`/`stopped_after_stage`/`failed`), `report_path` |
...
**`Store(url="sqlite:///findings.sqlite")`** — creates tables on init, runs
`_ensure_schema()` (backfills columns added after the first DB shipped via
`ALTER TABLE ... ADD COLUMN`, no Alembic), provides a transactional `session()`
and a no-commit `read_session()`.

- Writes: `create_run`, `finish_run` (records `outcome` + `finished_at`),
  `add_finding`, `record_validation`, `set_finding_status`, `add_wish`,
  `flush_tool_usage`, `set_wish_status`.
- Pipeline reads: `run_findings`, `upheld_findings(run_id)` (status
  `reach_upheld`), `finding_reasons`, `other_run_findings`, `tool_usage`.
- API read layer (paginated, `(rows, total)`): `list_runs`, `get_run`,
  `run_counts`, `query_findings` (status / severity / attack_class / stable_key
  + ordering), `list_validations`, `list_wishes` / `get_wish`,
  `tool_usage_by_tool`.
...
### 10.4 `crucible/api/` — the HTTP API  · *read endpoints done (milestone: API, PR #47); triggering done (milestone: Trigger)*

A FastAPI service (`crucible serve`, optional `crucible[api]` extra) that fans in
the three run-output sources behind one HTTP surface:

- **domain store** (`findings.sqlite`) via the `dao.py` read layer — runs,
  findings + provenance + funnel status, per-pass validation verdicts, wishlist,
  tool-usage;
- **execution state** (`checkpoints.sqlite`) via `graph.get_state` — `pending_hunts`,
  `cycle_count`, `recon_quality`, `subsystems`, next node;
- **workspace tree** via `api/artifacts.py` — a type-tagged index plus a
  traversal- and symlink-safe resolver (`architecture.md`, `report.*`,
  `recon/*.json`, `coverage/*.md`, `dedup/clusters.json`, `findings/*.json`,
  `offload/*.txt`, `run.log`).

| Module | Role |
|---|---|
| `app.py` | `create_app(settings)` — factory; store on `app.state`; router + exception-handler wiring |
| `settings.py` | `ApiSettings` — env-first; refuses a non-loopback bind with no token |
| `errors.py` | `ApiError` + handlers → `{error, detail}`, never a traceback |
...
**Read-first, with one deliberate exception.** `POST /wishes/{id}/resolve` and
`POST /runs` (milestone: Trigger) are the only writes, both gated behind the
write token. `POST /runs` is also the API's only outbound-reaching,
process-spawning action: it fetches a user-supplied git URL
(`crucible/repo_acquire.py` — `https://`-only, host allowlist, IP-literal /
loopback / link-local hard-block, git-argument-injection guard via `--`,
shallow clone, size/time caps) and launches `crucible run` as a detached
subprocess (`launcher.py`). Bearer-token auth (full + optional read-only) is
off on a loopback bind. Every other route makes **no outbound network calls**
— data residency (§13) holds for the read surface.

Contract suite: `tests/api/` (hermetic, no network) + a committed OpenAPI
snapshot (`tests/data/openapi.json`, regenerated by `scripts/dump-openapi.py`).
...
- **`crucible run --repo <path> [--workspace ...] [--checkpoint-db ...] [--resume <run_id> | --run-id <id>] [--stop-after <stage>]`**
  ...
  checkpoint. `--run-id <id>` (milestone: Trigger) starts fresh under a
  caller-chosen id — if a `Run` row already exists for it (the API launcher
  pre-registers one before the repo is even cloned), it's updated in place
  rather than re-inserted; mutually exclusive with `--resume`. `--stop-after
  <stage>` (a stage-node name — `recon`, `hunt`, `dedup`, …, case-insensitive)
  halts the run cleanly once that node completes, on the same exit path as a
  stub node (checkpoint written, exit 3); pass `--resume <run_id>` to continue
  past it.
...
- **`crucible serve [--host ...] [--port ...] [--store-url ...] [--checkpoint-db ...] [--workspace-root ...] [--no-auth] [--reload]`**
  Runs the read-first HTTP API (§10.4) under uvicorn. Needs the `crucible[api]`
  extra. See [`docs/api.md`](docs/api.md).
